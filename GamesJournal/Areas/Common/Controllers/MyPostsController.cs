...
{
    public class MyPostsController : BaseCommonController
    {
        // GET: Common/MyPosts
        [Authorize]
        public ActionResult Index(string SortOrder, string SortBy, string currentFilter, string searchString, string filterString, string Page)
        {
            ...
            var articles = objBs.ArticleBs.GetALL().Where(x => ( x.user.email == User.Identity.Name ||  x.user.username == User.Identity.Name) );
